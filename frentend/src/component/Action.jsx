...
const Action = ({ post }) => {
	...
	const handleLikeAndUnlike = async () => {
		if (!user) return showToast("ERROR", "Please login first to like a post", "error");
		if (isLiking) return;
		setIsLiking(true);
		try {
		...
		if (!user) return showToast("Error", "Login first to reply to a post", "error");
		if (loading) return;
		setLoading(true);
		try {
			const res = await fetch(`/api/posts/reply/${post._id}`, {
			method: "PUT", // Assuming it's a PUT request based on your original code
			headers: {
				"Content-Type": "application/json",
			},
		...
		<Flex flexDirection="column">
			...
			<Flex key={post._id} gap={3} alignItems="center">
			...
			<ModalContent>
				...
				<ModalFooter>
				<Button colorScheme="blue" mr={3} size="sm" isLoading={loading} onClick={handleReplay}>
					Reply
				</Button>
				</ModalFooter>
			</ModalContent>
