...
var ToastProvider = (props) => {
  const state = (0, import_react27.useSyncExternalStore)(
...
function useShortcut(props = {}) {
  ...
  const [keys, setKeys] = (0, import_react72.useState)([]);
...
function useShortcut2(props = {}) {
  const { timeout = 300, preventDefault = () => true } = props;
  const [keys, setKeys] = (0, import_react104.useState)([]);
...
function useEventListeners() {
  ...
  const currentListeners = listeners.current;
  const add = (0, import_react105.useCallback)((el, type, listener, options) => {
    ...
    (event) => {
      ...
      if (state2.step) {
        nextValue = parseFloat(
          roundValueToStep2(nextValue, state2.min, state2.step)
        );
      }
